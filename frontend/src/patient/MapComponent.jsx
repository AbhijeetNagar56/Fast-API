    ...
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-50">
        ...
        <div className="flex items-center gap-3">
            ...
            {[2, 5, 10].map((km) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  selectedRange === km ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                {km} KM
              </button>
          ...
          >
            {loading ? <span className="loading loading-spinner loading-xs"></span> : <Search size={16} />}
            Scan Area
          </button>
        </div>
      </header>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {filteredHospitals.length === 0 && !loading && (
              <div className="text-center py-10 opacity-50">
                ...
                <p className="text-sm">Click "Scan Area" to find <br/> nearby medical facilities.</p>
              </div>
            )}
            ...
            <MapContainer center={position} zoom={14} className="h-full w-full z-10">
              ...
              {filteredHospitals.map((hospital) => (
                <Marker
                  ...
                  icon={hospitalIcon}
                  ref={(ref) => ref && (markerRefs.current[hospital.id] = ref)}
                >
                  <Popup>
                    <div className="p-1">
                      <h4 className="font-bold text-slate-800 leading-tight">{hospital.name.split(',')[0]}</h4>
                      <p className="text-xs text-slate-500 mt-1">{hospital.distance.toFixed(2)} km away</p>
                      <button
